...
export function CalenderPage(){
    ...
    const pririoty = [
        {name:'High' , color:'red-200'},
        {name:'Medium' , color:'yellow-200'},
        {name:'Low' , color:'green-200'}
    ]
    ...
    return(
        <div className="h-full w-full justify-center flex p-3 overflow-y-scroll scrollbar-none">
            ...
            <div className="flex max-navlg:justify-center gap-2 w-[1100px] max-navlg:w-full h-full">
                <div className="flex h-full flex-col gap-2 min-w-[250px] max-navmd:min-w-[150px] max-navsm:min-w-[100px] max-navmd:hidden"> 
                   <div>
                    <button onClick={()=>setCreate(true)} className="flex hover:bg-[var(--overlay-color)] shadow-lg rounded-lg p-3 max-navmd:text-sm text4 bg-[var(--div-color)] max-navsm:text-[8px] max-navsm:p-2">
                        🚀 Create +
                    </button>
                   </div>
                    ...
                    <div className=" h-max w-full bg-[var(--popup-color)] p-2 rounded-md">
                        <div className="flex justify-between items-center gap-1 text4">
                            <div className="flex gap-1 text-[0.9vw] max-navlg:text-[12px] max-navsm:text-[10px] font-semibold">             
                                My Calender 
                            </div>
                            <div className="w-[20px] max-navsm:w-[10px]"><svg style={{fill: "var(--div-color)"}} xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path fill="currentColor" d="M480-360 280-560h400L480-360Z"/></svg></div>
                        </div>

                        <div className="flex-col mt-3 text-sm">
                            <div className="flex flex-col text-[0.8vw] max-navlg:text-[12px] max-navsm:text-[10px] gap-1">
                                { category.map((item , index)=>
                                <label key={index} class="relative flex items-center cursor-pointer gap-2">
                                    <input type="checkbox" class="hidden peer" />
                                    <div className="w-[0.9vw] h-[0.9vw] border-[1px] border-neutral-500 rounded-sm peer-checked:bg-red-500 peer-checked:border-red-500 transition-all"></div>
                    ...
                    <div className="bg-[var(--popup-color)] p-2 rounded-md">
                        <div className="flex justify-between items-center gap-1 text4">
                            <div className="flex gap-1 text-[0.9vw] max-navlg:text-[12px] max-navsm:text-[10px] font-semibold">                         
                                Pririoty
                            </div>
                        ...
                        <div className="flex-col mt-3 text-sm">
                            <div className="flex flex-col text-[0.8vw] max-navsm:text-[10px] max-navlg:text-[12px] gap-1">
                                { pririoty.map((item , index)=>
                                <label key={index} class="relative flex items-center cursor-pointer gap-2">
                                    <input type="checkbox" class="hidden peer" />
                                    <div className={`w-[0.9vw] h-[0.9vw] border-[1px] border-neutral-500 rounded-sm peer-checked:bg-red-500 peer-checked:border-red-500 transition-all`}></div>
                                    <span className="text-neutral-500">{item.name}</span>
                ...
                <div className="h-max w-[60vw] max-navlg:w-full bg-[var(--popup-color)] rounded-lg text4">
                    <div className="w-full h-auto p-5">
                        <FullCalendar
                            ...
                            events={[
                            { title: "Meeting", start: "2025-03-31T10:00:00",end:"2025-03-31T12:00:00"},
                            { title: "Lunch Break", start: "2025-03-31T13:00:00",end:"2025-03-31T15:00:00"},
                            ]}
                            eventContent={(eventInfo) => {
                            const { tag } = eventInfo.event.extendedProps;

                            return(
                            <div className="bg-red-500 w-full text-white p-1 rounded-md">
                                <div>{tag}</div>
